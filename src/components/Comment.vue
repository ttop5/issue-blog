...
export default {
  ...
  data() {
    return {
      comments: [],
      addCommentUrl: `https://github.com/${this.$store.getters.repositorySlug}/issues/${this.$route.params.id}/#new_comment_field`,
    };
  },
  filters: {
    timeAgo(d) {
  ...
  methods: {
    getComments() {
      axiosInstance.get(`/repos/${this.$store.getters.repositorySlug}/issues/${this.$route.params.id}/comments`)
        .then((res) => {
